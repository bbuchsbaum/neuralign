...
.gpca_fit <- function(data,
  ...
  gpca_args <- utils::modifyList(
    list(ncomp = ncomp, u = u, lambda = lambda),
    dots
  )

  # GPCA requires y (labels).  Use feature-level labels if available,
  # otherwise fall back to a constant label to run unsupervised.
  feat_labels <- train_data@metadata[["feature_labels"]]
  if (!is.null(feat_labels)) {
    gpca_args$y <- feat_labels
  }
  ...
  transforms <- .extract_operators_from_scores(
    gpca_result, data_list, ref$name, ref$data
  )
  ...
  list(
    ...
    method_state   = list(reference = ref$name, gpca_args = gpca_args)
  )
}
